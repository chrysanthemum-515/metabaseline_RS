...
def split_train(data):
    ...
    for idx in range(len(data)):
        num = len(data[idx])
        train[idx] = data[idx][:int(num*0.5)]
        val[idx] = data[idx][int(num*0.5):int(num*0.75)]
        test[idx] = data[idx][-int(num*0.75):]
    return train, val, test


catname2label = makecatname2label(path)
print(catname2label)
train_catname2label = {'airport': 0, 'bareland': 1, 'baseballfield': 2, 'beach': 3, 'bridge': 4,
                      'commercial': 5,  'denseresidential': 6, 'farmland': 7, 'meadow': 8, 'pond': 9,
                       'port': 10, 'railwaystation': 11, 'resort': 12, 'stadium': 13,'desert':14}
val_catname2label = {'mediumresidential': 0, 'mountain': 1, 'park': 2,
                     'parking': 3, 'playground':4} 
test_catname2label = {'river': 0, 'school': 1, 'sparseresidential': 2, 'square': 3,
                      'forest': 4, 'industrial': 5, 'center': 6, 'church': 7,
                      'storagetanks': 8, 'viaduct': 9}
...
with open('./meta-baseline/materials/AID-imagenet/AIDImageNet_category_split_train_phase_train.pickle', 'wb')as f:
    pickle.dump(d, f)
...
with open('./meta-baseline/materials/AID-imagenet/AIDImageNet_category_split_train_phase_val.pickle', 'wb')as f:
    pickle.dump(d, f)
...
with open('./meta-baseline/materials/AID-imagenet/AIDImageNet_category_split_train_phase_test.pickle', 'wb')as f:
    pickle.dump(d, f)
...
with open('./meta-baseline/materials/AID-imagenet/AIDImageNet_category_split_test.pickle', 'wb')as f:
    pickle.dump(d, f)
...
with open('./meta-baseline/materials/AID-imagenet/AIDImageNet_category_split_val.pickle', 'wb')as f:
    pickle.dump(d, f)
