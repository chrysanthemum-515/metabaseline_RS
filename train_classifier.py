...
def main(config):
    svname = args.name
    fs_ep_per_epoch = config['fs_ep_per_epoch']
    if svname is None:
    ...
    train_dataset = datasets.make(config['train_dataset'],
                                  **config['train_dataset_args'])  # train_phase_train
    ...
    if config.get('val_dataset'):
        ...
        val_dataset = datasets.make(config['val_dataset'],
                                    **config['val_dataset_args'])  # train_phase_val
    ...
    if config.get('fs_dataset'):
        ...
        for n_shot in n_shots:
            fs_sampler = CategoriesSampler(
                    ...
                    n_way, n_shot + n_query, ep_per_batch=fs_ep_per_epoch)
            #fs_batch个batch，每个batch包含4tasks，每个task有way*（shot+query)张图
    ...
    for epoch in range(1, max_epoch + 1 + 1):
        ...
        for data, label in tqdm(train_loader, desc='train', leave=False):
            data, label = data.cuda(), torch.LongTensor(label.numpy()).cuda()
            #拿出128张图计算logits
            logits = model(data)
            loss = lossf(logits, label.long())
            # loss = poly1_cross_entroy(logits,label)
            ...
            for i, n_shot in enumerate(n_shots):
                ...
                for data, _ in tqdm(fs_loaders[i],
                    ...
                    x_shot, x_query = fs.split_shot_query(
                            data.cuda(), n_way, n_shot, n_query, ep_per_batch=fs_ep_per_epoch)
                    #shot(4,5,1,3,80,80) query(4,75,3,80,80)
                    label = fs.make_nk_label(
                            n_way, n_query, ep_per_batch=fs_ep_per_epoch).cuda()
...
def poly1_cross_entroy(logits,labels,epsilon=1.0):
    lossf = nn.CrossEntropyLoss()
    pt = torch.sum(torch.mm(labels.reshape(1,-1).float(),F.softmax(logits,dim=-1)))
    CE = lossf(logits,labels)
    return CE+epsilon*(1-pt)
    

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--config',default='configs/train_classifier_NWPU.yaml')
